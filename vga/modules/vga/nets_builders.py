...
from modules.vga.layer_builders import ModulatedConv2d, NoiseInjection, Blur, Upsample, ModulatedConv2dRGB
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, bbox, obj_to_img, skip=None):
        out = self.conv(input, style, bbox, obj_to_img)
        out = out + self.bias
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, bbox, obj_to_img, noise=None):
        out = self.conv(input, style, bbox, obj_to_img)
...
class StyledConvNormal(nn.Module):
    def __init__(
            self,
            in_channel,
            out_channel,
    ...
    def forward(self, input, style, hair_mask=None, noise=None):
        out = self.conv(input, style, hair_mask=hair_mask)
        out = self.noise(out, noise=noise)
        # out = out + self.bias
        out = self.activate(out)
