...
def regular_mask(img, split):
    s = img.size()
    m = np.zeros((s[1], s[2])).astype(np.uint8)
    if split == 'train':
        t, l = random.randint(0, s[1] // 2), random.randint(0, s[2] // 2)
        m[t:t + s[1] // 2, l:l + s[2] // 2] = 255
    else:
        m[s[1] // 4:s[1] * 3 // 4, s[2] // 4:s[2] * 3 // 4] = 255
