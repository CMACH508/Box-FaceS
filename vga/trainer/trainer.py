...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            # Load data
            ##########################
            real_img, objs, boxes, obj_to_img, mask1, mask2 = \
                [tensor.cuda() for tensor in batch]
            ##########################
            # Update D network
            ##########################
            ...
            out, nodes, style = self.netE(real_img * mask1, real_img * mask2, objs, boxes, obj_to_img)
            fake_img = self.netG(out, nodes, style, boxes, obj_to_img)
    ...
    def check_model(self, loader):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(loader):
                ...
                # forward pass
                ##########################
                out, nodes, style = self.e_ema(real_img * mask1, real_img * mask2, objs, boxes, obj_to_img)
                recon = self.g_ema(out, nodes, style, boxes, obj_to_img)
